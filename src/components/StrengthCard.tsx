...
const deviconLogos: Partial<Record<DecorativeStrengthIcon, string>> = {
  azure:
...
function renderDecorativeIcon(icon: DecorativeStrengthIcon) {
  if (deviconLogos[icon]) {
    return (
      <span
        className="block size-6 bg-contain bg-center bg-no-repeat"
        style={{
          backgroundImage: `url(${deviconLogos[icon]})`,
        }}
      />
    );
  }
